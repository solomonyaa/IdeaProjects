...
public class AdventureGame {
    ...
    private void loadLocations(String data) {
        ...
        for (String row : data.split("\\R")) { //splits by rows (like "\n")
            String[] parts = row.split(",", 3);
            Arrays.asList(parts).replaceAll(String::trim); //erases spaces
    ...
    // The while loop in Main calls: move() --> play() --> visit() --> User Input...

    public void move(String direction) {

        var lastLocation = adventureMap.get(lastPlace);
        var nextPlaces = lastLocation.nextPlaces;
        String nextPlace;
